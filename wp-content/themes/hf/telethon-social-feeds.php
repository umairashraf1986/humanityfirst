...
if(!empty($_POST['load_th_feeds']) && $_POST['load_th_feeds']=='twitter_feeds'){
  ...
  $telethonEventID = isset($_POST['post_id']) ? $_POST['post_id'] : '';
  if($telethonEventID != '') {
    $twitterHashtag  = get_post_meta( $telethonEventID, 'hfusa-telethon_twitter_page',true );
  } else {
    $twitterHashtag = '#HFWeCare+OR+#HFUSA';
  }
  ...
  if(!empty($response->statuses) && is_array($response->statuses)){
    ...
    foreach ($response->statuses as $post){
      ...
      $picture = isset($post->entities->media[0]->media_url_https) ? $post->entities->media[0]->media_url_https : '';
...
if(!empty($_POST['load_th_feeds']) && $_POST['load_th_feeds']=='youtube_feeds'){

  $htmlString = ' ';
  $telethonEventID = isset($_POST['post_id']) ? $_POST['post_id'] : '';
  $token = isset($_POST['token']) ? $_POST['token'] : '';
  $videoId  = get_post_meta( $telethonEventID, 'hfusa-telethon_video_id',true );
  $key = get_option( 'hf_youtube_api_key' );

  $yourubeVideoComments = 'https://www.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&maxResults=20&pageToken='.$token.'&videoId='.$videoId.'&key='.$key;
  ...
  if($totalResults > 0){
    ...
    if(!empty($items) && is_array($items)){
      ...
      foreach($items as $item){
        ...
        $htmlString .='<div class="t-individual-comment">
          <img alt="" src="'.$authorProfileImageUrl.'" width="24" height="24">
          <span><a target="_blank" href="'.$authorChannelUrl.'"><strong>'.$authorDisplayName.'</strong></a></span>
          <span>'.$textDisplay.'</span>
        </div>';
      }
      
      $htmlString .='<div data-next-page-token="'.$nextPageToken.'" class="t-next-page-token"></div>';
