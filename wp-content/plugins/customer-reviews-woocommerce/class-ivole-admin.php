...
if ( ! class_exists( 'Ivole_Admin' ) ) :
	...
	class Ivole_Admin {
	  ...
	  public function __construct() {
		    ...
		    $this->id    = 'ivole';
				$this->label = __( 'Reviews', IVOLE_TEXT_DOMAIN );
				$this->ver = '3.35';
			...
			if ( 'ivole_reviews' == $current_section ) {
				$settings = array(
					...
					array(
						...
						'desc'          => __( 'Enable attachment of images to reviews left on WooCommerce product pages. If you would like to enable attachment of images on aggregated review forms, this can be done <a href="' . admin_url( 'admin.php?page=wc-settings&tab=ivole&section' ) . '">here</a>.', IVOLE_TEXT_DOMAIN ),
		        ...
		        'id' => 'ivole_captcha_secret_key',
					...
					array(
						...
						'This shortcode supports arguments: [cusrev_all_reviews sort="DESC" per_page="10" number="-1" show_summary_bar="true" show_pictures="false" show_products="true"]. ' .
						...
						'and specifies if product names along with product thumbnails should be shown for each review.', IVOLE_TEXT_DOMAIN ),
					...
					array(
						...
						'desc'          => __( 'Enable this option to show verified badges <img src="' . untrailingslashit( plugin_dir_url( __FILE__ ) ) . '/img/shield-20.png" style="width:17px;"> on reviews and gain your customer\'s confidence. Badges will be shown only for reviews submitted via this plugin. Each badge will contain a nofollow link to a verified copy of a review.', IVOLE_TEXT_DOMAIN ),
				...
				$settings = array(
					array(
		        		...
		        		'type' => 'title',
		        		'desc' => __( 'You can unlock premium features of the Customer Reviews plugin by purchasing a license key => <a href="https://www.cusrev.com/" target="_blank">Buy License Key</a>', IVOLE_TEXT_DOMAIN ),
		        		'id' => 'ivole_options'
		      ),
					array(
				...
				if( function_exists( 'qtranxf_useCurrentLanguageIfNotFoundUseDefaultLanguage' ) ) {
					$language_desc = $language_desc . ' ' . __( 'It looks like you have qTranslate-X plugin activated. You might want to choose "qTranslate-X Automatic" option to enable automatic selection of language.', IVOLE_TEXT_DOMAIN );
					$available_languages = array( 'QQ' => __( 'qTranslate-X Automatic', IVOLE_TEXT_DOMAIN ) ) + $available_languages;
				}
				//WPML integration
				if ( defined( 'ICL_LANGUAGE_CODE' ) ) {
					$language_desc = $language_desc . ' ' . __( 'It looks like you have WPML plugin activated. You might want to choose "WPML Automatic" option to enable automate selection of language.', IVOLE_TEXT_DOMAIN );
		      ...
		      array(
        		...
        		'type' => 'title',
        		'desc' => __( 'Settings for WooCommerce Customer Reviews plugin. Configure WooCommerce to send automatic follow-up emails (reminders) that gather product reviews. Before enabling this plugin you MUST update your terms and conditions and make sure that your customers consent to receive an invitation to review their order. By enabling and using this plugin, you agree to <a href="https://www.cusrev.com/terms.html" target="_blank">terms and conditions</a>.', IVOLE_TEXT_DOMAIN ),
        		'id' => 'ivole_options'
		      ),
		      array(
						'title'         => __( 'Enable Automatic Reminders', IVOLE_TEXT_DOMAIN ),
						'desc'          => __( 'Enable automatic follow-up emails with a reminder to submit a review.', IVOLE_TEXT_DOMAIN ),
						'id'            => 'ivole_enable',
						'default'       => 'no',
						'type'          => 'checkbox'
					),
		...
		public function output() {
			...
			$external_sections = array( 'ivole_coupons', 'ivole_diagnostics', 'ivole_import' );

			if( ! in_array( $current_section, $external_sections ) ) {
		...
		public function show_cselect($value) {
			...
			?><tr valign="top">
				...
				<td class="forminp">
					<select multiple="multiple" name="<?php echo esc_attr( $value['id'] ); ?>[]" style="width:350px" data-placeholder="<?php esc_attr_e( 'Choose product categories&hellip;', IVOLE_TEXT_DOMAIN ); ?>" aria-label="<?php esc_attr_e( 'Category', IVOLE_TEXT_DOMAIN ) ?>" class="wc-enhanced-select">
		...
		public function show_upload_shop_logo( $value ) {
			...
			?><tr valign="top">
				...
				<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
					...
					<?php $remove_button_style = ( get_option( 'ivole_shop_logo', '' ) ) ? 'visibility:visible' : 'visibility:hidden'; ?>
					<button
					...
					><?php _e( 'Remove logo', IVOLE_TEXT_DOMAIN ); ?></button>
				</td>
			</tr>
			<?php
		}
			...
			jQuery(document).ready(function($) {
				...
				if( jQuery('#ivole_license_status').length > 0 ) {
					...
					jQuery.post(ajaxurl, data, function(response) {
						...
						if( '<?php echo __( 'Active', IVOLE_TEXT_DOMAIN ); ?>' === response.message ) {
				...
				if( jQuery('#ivole_email_from').length > 0 ) {
					...
					jQuery.post(ajaxurl, data, function(response) {
						...
						if( '<?php echo __( 'Active', IVOLE_TEXT_DOMAIN ); ?>' === response.license ) {
		...
		public function check_license_email_ajax() {
			$license = new Ivole_License();
			$lval = $license->check_license();
			if( __( 'Active', IVOLE_TEXT_DOMAIN ) == $lval ) {
		...
		public function admin_notice_install() {
			if( current_user_can('manage_options') ) {
				...
				$message = sprintf( __( '<strong>Customer Reviews for WooCommerce</strong> plugin has been activated. Please go to <a href="%s">WooCommerce settings</a> and configure this plugin to start receiving more authentic reviews!', IVOLE_TEXT_DOMAIN ), $settings_url );
		...
		public function get_sections() {
		    $sections = array(
		        '' => __( 'Review Reminder', IVOLE_TEXT_DOMAIN ),
		        'ivole_reviews' => __( 'Review Extensions', IVOLE_TEXT_DOMAIN ),
						'ivole_coupons' => __( 'Review for Discount', IVOLE_TEXT_DOMAIN ),
						'ivole_premium' => __( '&#9733; Premium &#9733;', IVOLE_TEXT_DOMAIN ),
						'ivole_diagnostics' => __( 'Diagnostics', IVOLE_TEXT_DOMAIN ),
						'ivole_import' => __( 'Import Reviews', IVOLE_TEXT_DOMAIN )
		    );
		    return $sections;
		}
