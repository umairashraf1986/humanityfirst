...
class WC_Name_Your_Price_Display {
	...
	public function display_minimum_price( $product = false ){
		...
		// If not NYP quit right now of if Min price is hidden.
		if( ! $product || ( ! WC_Name_Your_Price_Helpers::is_nyp( $product ) && ! WC_Name_Your_Price_Helpers::has_nyp( $product ) ) || WC_Name_Your_Price_Helpers::is_minimum_hidden( $product ) ) {
			return;
		}
